...
- (void)longPress {
    ...
    UIAlertAction *confirm = [UIAlertAction actionWithTitle:@"保存图片" style:UIAlertActionStyleDestructive handler:^(UIAlertAction * _Nonnull action) {
        
        NSLog(@"确认保存图片");
        
        // 保存图片到相册
        UIImageWriteToSavedPhotosAlbum(tile.imageView.image,self,@selector(imageSavedToPhotosAlbum:didFinishSavingWithError:contextInfo:),nil);
    ...
    [toVC presentViewController:alertControl animated:YES completion:nil];
