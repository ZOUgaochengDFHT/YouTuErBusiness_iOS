 ...
 *  展示，提供单选多选
 */
@interface YTEEMChooseViewController : UIViewController<UITableViewDataSource, UITableViewDelegate>
{
 ...
 *  页面加载完成的回调
 */
@property (copy) void (^viewDidLoadBlock)(YTEEMChooseViewController *controller);

/**
 *  获取数据源的回调 (也可通过实现代理方法中的[viewControllerLoadDataSource:])
 */
@property (copy) void (^loadDataSourceBlock)(YTEEMChooseViewController *controller);
 ...
 *  @return 数据源
 */
- (NSArray *)viewControllerLoadDataSource:(YTEEMChooseViewController *)viewController;
 ...
 *  @return 是否隐藏页面
 */
- (BOOL)viewController:(YTEEMChooseViewController *)viewController didFinishSelectedSources:(NSArray *)selectedSources;
 ...
 *  @param viewController  列表视图
 */
- (void)viewControllerDidSelectBack:(YTEEMChooseViewController *)viewController;
